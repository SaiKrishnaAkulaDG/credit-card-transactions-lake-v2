...
description: >
  ...
  (Tier 3) also load bce_signal.md.
---

# PBVI Core Skill — v4.3
...
| `tools/` | Agentic build automation scripts — launch.sh, resume_session.sh, resume_challenge.sh, challenge.sh, monitor.sh. Methodology artifacts under version control. No source code, no planning artifacts. |
...
If both checks fail, output this block and continue to STEP 2:
  ...
  Before running any autonomous session, copy these five files from the DG-OS repository
  into this project's tools/ directory and run chmod +x tools/*.sh:
    tools/launch.sh
...
| discovery/ | BCE SIL artifacts + discovery/components/ for component files |
| enhancements/ | REGISTRY.md + ENH-NNN subdirectory per enhancement |
| tools/ | Agentic build automation scripts — launch.sh, resume_session.sh, resume_challenge.sh, challenge.sh, monitor.sh |
...
6. Regression classification — REGRESSION-RELEVANT or NOT-REGRESSION-RELEVANT, with
   a one-line rationale. Regression-relevant tasks must carry portable verification
   commands: runnable from repo root without session-specific setup. AI classifies at
   generation time; engineer confirms at sign-off.
...
7. Regression suite assembly — collect the portable verification commands from all
   REGRESSION-RELEVANT tasks in EXECUTION_PLAN.md. Consolidate into a single runnable
   suite at `verification/REGRESSION_SUITE.sh`. Commit to repo alongside BCE artifacts.
...
`verification/REGRESSION_SUITE.sh`, all seven BCE artifacts committed,
...
**Autonomous mode:** `./tools/launch.sh` reads the session prompt file and feeds it
to the agent. Do not paste the prompt manually.
